...
<head>
	...
	<title>Admin Product Management | Add New Product</title>
									    ...
									    <% for (int i = 0; i < 8; i++) { %>
									        <div class="col-md-3 px-md-1">
									            <div class="form-group file-upload">
									                ...
									                <div class="file-select">
									                    <div class="file-select-button" id="fileName<%= i + 1 %>">Image</div>
									                    <div class="file-select-name" id="noFile<%= i + 1 %>">No file chosen...</div> 
									                    <input id="imageFiles<%= i + 1 %>" name="imageFiles" type="file" class="form-control-file" multiple="multiple">
									                </div>
									            </div>
									        </div>
									    <% } %>
	...
	<script>
		...
		$(document).ready(function() {
	        $('input[type="file"]').on('change', function() {
	            ...
	            var fileId = inputId.replace('imageFiles', '');
	            var filename = $(this).val();
	            if (/^\s*$/.test(filename)) {
	                $(".file-upload").removeClass('active');
	                $("#noFile" + fileId).text("No file chosen..."); 
	            } else {
	                $(".file-upload").addClass('active');
	                $("#noFile" + fileId).text(filename.replace("C:\\fakepath\\", ""));
	            }
	        });
	    });
	</script>
